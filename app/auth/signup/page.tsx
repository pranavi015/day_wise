...
export default function SignupPage() {
  ...
  const strength = Math.min(100, password.length * 10 + (/[A-Z]/.test(password) ? 10 : 0) + (/[0-9]/.test(password) ? 10 : 0));
    ...
    <div style={{ 
      ...
      <div className="modal-enter" style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ fontSize: 14, color: "var(--text-tertiary)" }}>Create your account to start learning.</p>
        </div>
        ...
        <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border-subtle)", borderRadius: 16, padding: "32px", boxShadow: "var(--shadow-xl)" }}>
          <form onSubmit={handleSignup}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ position: "relative" }}>
                <User size={16} color={focusedField === "name" ? "var(--accent)" : "var(--text-tertiary)"} style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", transition: "color 200ms" }} />
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ marginTop: 10 }}>
                <div style={{ display: "flex", gap: 4, height: 4, marginBottom: 8 }}>
                  <div style={{ flex: 1, borderRadius: 2, background: password ? getStrengthColor() : "var(--border-subtle)", opacity: password ? 1 : 0.5, transition: "all 300ms ease" }} />
                  <div style={{ flex: 1, borderRadius: 2, background: strength >= 40 ? getStrengthColor() : "var(--border-subtle)", transition: "all 300ms ease" }} />
                  <div style={{ flex: 1, borderRadius: 2, background: strength >= 70 ? getStrengthColor() : "var(--border-subtle)", transition: "all 300ms ease" }} />
                </div>
                <p style={{ fontSize: 11, color: "var(--text-tertiary)", margin: 0, display: "flex", gap: 4, alignItems: "center" }}>
                  {strength >= 70 ? <><Check size={12} color="var(--success)"/> Strong password</> : "Must be at least 8 characters."}
                </p>
              </div>
            </div>
          ...
          <button
            ...
            {googleLoading
              ...
              : <><svg width="18" height="18" viewBox="0 0 18 18"><path fill="#4285F4" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 01-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z"/><path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 009 18z"/><path fill="#FBBC05" d="M3.964 10.71A5.41 5.41 0 013.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 000 9c0 1.452.348 2.827.957 4.042l3.007-2.332z"/><path fill="#EA4335" d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 00.957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z"/></svg>Sign up with Google</>}
          </button>
        </div>

        <p style={{ textAlign: "center", fontSize: 13, color: "var(--text-tertiary)", marginTop: 24 }}>
          Already have an account?{" "}
          <Link href="/auth/login" style={{ color: "var(--accent)", fontWeight: 600, textDecoration: "none" }}>Log in</Link>
        </p>
      </div>
